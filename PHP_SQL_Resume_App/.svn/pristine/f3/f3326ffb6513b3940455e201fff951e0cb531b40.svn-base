...
{   
    ...
    $userName = $_REQUEST ['login'];//login of the user
    $resumeName = $_REQUEST ['name'];//name of the resume
    ...
    try
    {
        ...
        $q = "SELECT ID, RealName FROM ps5_User WHERE UserName=?" ;
...
foreach ($userObj->resumeArray as $key => $v)
        {
    if($v->Name = $resumeName)
    {
        $resumeNumber = $key;
        
    }
    
        }//end find resumenumber
