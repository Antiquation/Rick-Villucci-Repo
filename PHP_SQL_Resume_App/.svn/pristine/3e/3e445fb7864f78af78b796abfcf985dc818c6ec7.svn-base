...
//________________________________________________________________________________
    ...
    try
    {
        ...
        if($errClass =="")
        {
            ...
            { 
                //user is authenticated save to the DB
                if(!$isGuest)
                {
                    ...
                    //and update session
                    $create = "INSERT INTO `ps5_Job_Description` (`Name`, `StartMonth`, `StartYear`, `EndMonth`, `EndYear`, `JobDescription`) VALUES (?, ?, ?, ?, ?, ?)";
                    ...
                    //insert into Job
                    $create = "INSERT INTO `ps5_Job` (`ID`, `JobNumber`, `Name`) VALUES (?, ?, ?)";
                    $stmt = $db->prepare ( $create);//prepare the query
                    //do insert  
                    $stmt->bindValue(1,$ID);
                ...
                {
                    //the user is a guest - just save to the session
                    
                    //THEN SAVE to THE SEESSION
                    $userObj->jobArray[] = $TempJob; //replaces the item at that location in the session with the new item
                    $message ="New Job: $Name Saved";//message to the user
            ...
            elseif(isset ( $_REQUEST ['Over_Write'] ))
            {
                ...
                if(!$isGuest)
                {
                    ...
                    $over = "UPDATE ps5_Job_Description SET Name=?, StartMonth=?, StartYear=?, EndMonth=?, EndYear=?, JobDescription=? WHERE JobNumber=?";
    ...
    if ( isset($_REQUEST ['Delete_Job']))
    {
        ...
        try
        {
            ...
            if(!$isGuest)
            {
                ...
                $deleteMe = "DELETE FROM ps5_Job_Description WHERE JobNumber = ?";
    ...
    {
        $q = "SELECT JobNumber FROM ps5_Job_Description WHERE Name=? and StartMonth=? and JobDescription=?";
...
//all of these variables are references of the original calling variables so no return is needed
    ...
    function checkYearValue(&$shown, &$error,&$errClass,$toBChecked)//clean
{ 
...
//all of these variables are references of the original calling variables so no return is needed
    function checkOptionValue(&$shown,&$error,$toBChecked)
    {
            ...
            if (strlen($toBChecked) > 0 && $toBChecked != "defaultMonth")
		    {
...
<table class='Resumes'>
 <tr>
    ...
    <td><input type='submit' id ='editBtn' hidden  name='View_Edit' value='View/Edit'/></td>
...
<tr>
 ...
 <tr>
  ...
  <tr>
    ...
    <td><input type='text' id='StartYear'  class ='hide' maxlength='4' size='4' id='StartYear' name='StartYear'  value='$StartYear'/>$syErr</td>
  </tr>
   <tr>
  ...
  <tr>
    ...
    <td><input type='text'  id='EndYear' class ='hide' maxlength='4' size='4' id='EndYear' name='EndYear' value='$EndYear'/>$eyErr</td>
    <td><div id='endyearmessage'></div></td>
    
  </tr>
   </tr>
  <tr>
  <td class ='hide'> Job Description </td>
  <td colspan ='2'><textarea spellcheck='true' class ='hide' class='$errClass' id='JobDescription' maxlength='700' name='JobDescription' 
        placeholder='ENTER POSITION DESCRIPTION' rows='4' cols='50' >$JobDescription</textarea>$jobDErr</td>
  </tr>
  <tr>
    <td>
    <p>
    <input type='button' class ='hide' disabled id='addressSubmit' value='Submit Job'/>
    </p>
</td>
    <td><p>
