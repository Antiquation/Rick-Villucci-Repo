    ...
    public partial class Form1 : Form
    {
        ...
        private void displaySelection(SpreadsheetPanel ss)
        {
            ...
            {       //sets the editable field to the content of the cell
                ...
                //variables for restoring the last cells value
            lastCell = CurrentCellName;
            lastCol = col;
            lastRow = row;

                //indicates that the file needs to be saved
        ...
        {
                //check if the file has been saved and then close the program
            closeMe();
        ...
        //    /// <summary>
        //    /// Method to check and ensure that the user is saving his file before closing
        //    /// </summary>
        //    /// <param name="sender"></param>
        //    /// <param name="e"></param>
        //private void Form1_FormClosing(object sender, FormClosingEventArgs e)
        //{
        //    if (sheet.Changed == true && e.CloseReason == CloseReason.UserClosing)
        //    {
        //        DialogResult S = MessageBox.Show("Save File Before Close?", "File Not Saved", MessageBoxButtons.YesNo, MessageBoxIcon.Question);
        ...
        //    }
        //    else if (sheet.Changed == true && e.CloseReason == CloseReason.WindowsShutDown)
        //    {
        //        DialogResult S = MessageBox.Show("Save File Before Close?", "File Not Saved", MessageBoxButtons.YesNo, MessageBoxIcon.Question);
        ...
        private void openToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            if (DialogResult == DialogResult.OK)
            {
                ...
                SheetApplicationContext.getAppContext().RunForm(new Form1(open.FileName));
            }
        ...
        {
            if (sheet.Changed == true && sheet.fileName != null)
            {
                sheet.Save(sheet.fileName);

                    //takes off the not saved indicator
                char[] trimMe = { '*' };
                    foreach(char c in this.Text)
                    {
                        this.Text = this.Text.TrimEnd(trimMe);
                    }
                
                
            }

            else if (sheet.Changed == true && sheet.fileName == null)
            {
                saveMe();
            }
        }//end save method
        ...
        private void howToUseThisProgramToolStripMenuItem_Click(object sender, EventArgs e)
        {
            MessageBox.Show("When opening this application it will be necessary to FIRST select a cell with your mouse /n " +
                "Then to add a value or a formula to the selected cell click in the box marked FX.../n" +
                    " next press the ENTER kee to load the value into the cell permanently... or press escape if you change your mind.../n" +
                        "If the boxes turn green you know the values loaded succesfully... If any of the boxes turn red there is an error message - read and fix.../n" +
            "if your file is not saved you will see an asteric next to the file name... If you wish to save or open a new or preexisting file use the file menu.");
                
        }//end closeme helper method
        ...
        {
            //DialogResult S = MessageBox.Show("Save File Before Close?", "File Not Saved", MessageBoxButtons.YesNoCancel, MessageBoxIcon.Question);
