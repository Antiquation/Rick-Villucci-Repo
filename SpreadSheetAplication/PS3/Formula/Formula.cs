...
namespace SpreadsheetUtilities
{
    ...
    //*************************************
            ...
            foreach(string token in GetTokens(formula))
            {
                    ...
                    //INTRA-ITERATIVE EXCEPTION CHECKING*******************
                                ...
                                //if the token is not a verified operator
                               if (!((Regex.IsMatch(token, @"^[a-zA-Z]+\d+$")) || ((parseMe(token)) || (Regex.IsMatch(token, @"^[\+\-*/()]+$")))))
                            {
                                throw new FormulaFormatException("Invalid Formula Expression - Check for Typos");
                            }
                            ...
                            if (tokenC == 0)
                            {
                                ...
                                if (!((Regex.IsMatch(token, @"^[a-zA-Z]+\d+$")) || (parseMe(token)) || (Regex.IsMatch(token, @"^[(]+$"))))
                                {
                                    throw new FormulaFormatException("Formula must begin with either a Variable, Number, or Open Perenthesis");
                                }
                            ...
                            if (lastT != null)
                            {
                                //if the last token was an "(" or an operator
                                if (((Regex.IsMatch(lastT, @"^[\+\-*/(]+$")) || (Regex.IsMatch(lastT, @"^[(]+$"))))
                                {
                                    ...
                                    if (!((Regex.IsMatch(token, @"^[a-zA-Z]+\d+$")) || (parseMe(token)) || (Regex.IsMatch(token, @"^[(]+$"))))
                                    {
                                        throw new FormulaFormatException("The item immediately proceeding the Open Perenthesis must be either a variable, number or an open perenthesis.");
                                    }
                                }//end if lastT is "("
                                ...
                                else if (((Regex.IsMatch(lastT, @"^[a-zA-Z]+\d+$")) || (parseMe(lastT)) || (Regex.IsMatch(lastT, @"^[)]+$"))))
                                {
                                    //if current token is NOT an operator or a closing perenthesis throw exception
                                    if (!((Regex.IsMatch(token, @"^[\+\-*/]+$")) || (Regex.IsMatch(token, @"^[)]+$"))))
                                        throw new FormulaFormatException("The item immediately folowing a variable, number or closing perenthesis must be a closing perenthesis or an operator.");
                                }
                            ...
                            if (closeP > openP)
                                throw new FormulaFormatException("The number of closing perenthesis cannot exceed the number of opening");
                ...
                //POST ITERATIVE FORMULA CHECKS***********
                            ...
                            if (!(tokenC >= 1))//*** PROGRAMING NOTE *** may need to change to tokenC != 2 if fails the must have one token test
                                throw new FormulaFormatException("There must be at least one valid token in your expression");
                           ...
                           // string lT = Char.IsLetter(lastT[0]);
                            ...
                            if (!((Char.IsLetter(lastT[0])) || ((parseMe(lastT)) || (Regex.IsMatch(lastT, @"^[)]+$")))))
                                throw new FormulaFormatException("Formula must end with either a Variable, Number, or Closing Perenthesis");
                            
      
                                //checks that the number of opening and closing perenthesis are equal
                            if(openP != closeP)
                                throw new FormulaFormatException("The number of closing perenthesis MUST EQUAL the number of opening");
    ...
    //*********************CLOSED BRACKET TOKEN PROCESSING AND PASSING TO SECOND LEVEL BRACKET PROCESSING
                    ...
                    else if(Value.Count == 2 && Operator.Count == 1 && Operator.Peek() == "+" || Operator.Peek() == "-")
                    {
                        ...
                        throw new FormulaError("Incorrect Mathmatic Expression");
    ...
    /// </summary>
    public class FormulaError : Exception
    {
        /// <summary>
